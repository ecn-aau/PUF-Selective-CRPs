# -*- coding: utf-8 -*-
"""
Created on Thu May 25 11:39:12 2023

@author: Mieszko Ferens

Script to run an experiment for modelling an (1,k)-Interpose PUF that uses
regular patterns (BP CRPs) as CRPs during authentication with the server.
"""
...
def log_data_to_csv(args, iterations, accuracy):
    data = pd.DataFrame({"seed": [args.seed],
                         ...
                         "accuracy": [accuracy]})
    filepath = Path(args.outdir + "out_regular_pattern_1" + str(args.k_down) +
                    "IPUF.csv")
    if(filepath.is_file()):
...
def main():
    ...
    parser.add_argument("--ML-algorithm", type=str, default="MLP",
    ...
    challenges_up = create_binary_code_challenges(n=args.n_bits, N=N)
